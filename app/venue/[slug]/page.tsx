...
import { MapPin, ChefHat, Star, ArrowLeft } from 'lucide-react';
  ...
  return (
    <div className="min-h-screen bg-background pb-24 lg:pb-8">
      ...
      <header className="sticky top-0 z-40 glass border-b border-border/30">
        <div className="mx-auto flex h-14 items-center gap-4 px-4 max-w-5xl">
          ...
          {venue.cuisines && venue.cuisines.length > 0 && (
            <div className="flex flex-wrap gap-2">
              {venue.cuisines.map((c) => (
                <Badge key={c.id} variant="secondary" className="text-xs flex items-center gap-1.5">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          <div className="md:col-span-1 space-y-6">
            ...
            <div className="space-y-2">
               {stats.histogram.sort((a, b) => b.rating - a.rating).map((item) => {
                 ...
                 return (
                   <div key={item.rating} className="flex items-center gap-2 text-xs">
                     <span className="w-8 text-right font-medium">{item.rating}</span>
                     <Progress value={percentage} className="h-2" />
                     <span className="w-8 text-muted-foreground tabular-nums">{item.count}</span>
                   </div>
                 );
               })}
            </div>
