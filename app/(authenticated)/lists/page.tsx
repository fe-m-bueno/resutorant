...
export default function ListsPage() {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      // Fetch venue counts for each list (pass user.id as viewerId to see all items)
      const counts: Record<string, number> = {};
      await Promise.all(
        listsData.map(async (list) => {
          const listDetails = await getListDetails(list.id, user.id);
      ...
      <main className="pb-24 lg:pb-8">
        <div className="mx-auto max-w-3xl px-4 lg:px-8 py-6 lg:py-8">
          <div className="flex items-center justify-between mb-6">
            <h1 className="text-2xl font-bold lg:hidden">Minhas Listas</h1>
            <Button
            ...
            {isLoading ? (
              ...
              <div className="text-center py-16 px-4">
                ...
                <p className="text-muted-foreground text-sm mb-6 max-w-sm mx-auto">
                  Crie listas para organizar seus lugares favoritos, locais que
                  quer visitar e muito mais
                </p>
                <Button
                  onClick={() => setIsCreateListModalOpen(true)}
                  size="lg"
