...
import { getListDetails, reorderListItems, removeListItem, deleteList } from "@/lib/queries"
...
export default function ListDetailsPage() {
  ...
  const loadData = async () => {
    try {
      ...
      const data = await getListDetails(id as string)
      setList(data)
      
      // Determine initial tab based on content availability or settings
      if (data) {
  ...
  if (!list) {
      return (
          <div className="flex flex-col items-center justify-center min-h-screen">
              ...
              <Button variant="link" onClick={() => router.push("/lists")}>Voltar para listas</Button>
          </div>
      )
  }
  ...
  return (
    <div className="min-h-screen bg-background pb-24 md:pb-8 lg:pl-64">
      <BottomNav onAddClick={() => setIsLogModalOpen(true)} />
      {/* Header */}
      <header className="sticky top-0 z-40 glass border-b border-border/30">
        <div className="mx-auto max-w-3xl flex h-14 items-center justify-between px-4">
          <div className="flex items-center gap-2">
            ...
            <h1 className="font-semibold text-lg truncate max-w-[200px]">{list.name}</h1>
          </div>
      ...
      <main className="mx-auto max-w-3xl p-4 lg:p-8 space-y-6">
          {/* List Header Info */}
          <div className="flex items-start gap-4">
              ...
              <div className="flex-1 space-y-1">
                  ...
                  <div className="flex items-center gap-2 text-xs text-muted-foreground pt-1">
                        <Link href={`/@${(list as any).author?.username}`} className="hover:underline">
                             @{ (list as any).author?.username }
                        </Link>
                        <span>•</span>
                        {list.is_public ? (
                            <span className="flex items-center gap-1"><Globe className="h-3 w-3"/> Pública</span>
                        ) : (
                            <span className="flex items-center gap-1"><Lock className="h-3 w-3"/> Privada</span>
                        )}
                        <span>•</span>
              ...
              {isOwner && (
                 <>
                  {isReordering ? (
                      ...
                      <>
                        <Button variant="outline" onClick={startReordering} className="gap-2">
          ...
          <Tabs value={activeTab} onValueChange={setActiveTab}>
              <TabsList>
                  {list.show_places && (
                      <TabsTrigger value="places">Lugares <Badge variant="secondary" className="ml-2 text-[10px] h-4">{placeItems.length}</Badge></TabsTrigger>
                  )}
                  {list.show_reviews && (
                      <TabsTrigger value="reviews">Reviews <Badge variant="secondary" className="ml-2 text-[10px] h-4">{reviewItems.length}</Badge></TabsTrigger>
                  )}
              </TabsList>
                      ...
                      {list.show_places && placeItems.length > 0 ? (
                          isReordering && isOwner ? (
                               <DraggableList
                                  items={placeItems}
                                  className="space-y-4"
                                  onReorder={(items) => handleReorder([...items, ...reviewItems])}
                                ...
                                renderItem={(item, isDragging) => (
                                    <div className="group relative">
                                        ...
                                        {isOwner && !isDragging && (
                                            <Button 
                                                variant="destructive" size="icon" className="absolute -right-2 -top-2 h-6 w-6 rounded-full opacity-0 group-hover:opacity-100 z-10 scale-75 shadow-sm"
                        ...
                        <div className="text-center py-10 text-muted-foreground">
                            {list.show_places ? "Nenhum lugar nesta lista" : "A visualização de lugares está desativada"}
                        </div>
                    )}
              </TabsContent>
                   ...
                   {list.show_reviews && reviewItems.length > 0 ? (
                        isReordering && isOwner ? (
                             <DraggableList
                                items={reviewItems}
                                className="space-y-4"
                                onReorder={(items) => handleReorder([...placeItems, ...items])}
                                renderItem={(item, isDragging) => (
                                    <div className="group relative">
                                        <div className="absolute -left-3 top-1/2 -translate-y-1/2 opacity-0 group-hover:opacity-100 cursor-grab active:cursor-grabbing p-1">
                                            <GripVertical className="h-4 w-4 text-muted-foreground"/>
                                        ...
                                        {list.is_ordered && (
                                            <div className="absolute -bottom-2 left-1/2 -translate-x-1/2 z-10 bg-orange-500 text-white text-[10px] font-bold h-6 w-6 rounded-full flex items-center justify-center shadow-sm border border-background">
                                                {item.position + 1} // Logic for global numbering? If tabs are split, numbering might be confusing if global. 
                         ...
                         <div className="text-center py-10 text-muted-foreground">
                            {list.show_reviews ? "Nenhum review nesta lista" : "A visualização de reviews está desativada"}
                        </div>
                    )}
              </TabsContent>
          </Tabs>
